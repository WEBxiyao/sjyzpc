...
<script>
import { listMaterial, getMaterial, delMaterial, addMaterial, updateMaterial, exportMaterial, audit } from "@/api/material";
...
export default {
  ...
  filters: {
    publish(value){
     ...
     if(this.publishTypeOptions.length>0){
       index=this.publishTypeOptions.findIndex(v=>v.dictValue==value)
     }
      if(index!==-1){
    ...
    materialClass(value){
      let index=this.typeOptions.findIndex(v=>v.dictValue==value)
      if(index!==-1){
  ...
  methods: {
    ...
    handlePreview(row){
      this.rowObj=row
      console.log(row,"-----row-------",this.fblx)
      this.previewShow = true
      this.previewhtml = row.content
    },
    //审核
    handleExamine(row){
      console.log("row",row)
    ...
    submitForm: function() {
      ...
      this.ExamineData.publishStatus = this.form.publishStatus

      if(this.ExamineData){
        audit(this.ExamineData).then(res=>{
          // console.log(res)
          this.getList();
          if(res.code === 200){
            this.$message({
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除素材内容编号为"' + ids + '"的数据项?', "警告", {
